...
class UserTagLib {
    ...
    def userSelectForOperator = { attrs ->
        ...
        def duplicates = User.executeQuery('select realName from User group by realName having count(realName) > 1')

        def from = [] as List
        User.findAllByOrganization(organization, [sort: 'realName', order: 'asc']).each {
