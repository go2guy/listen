      ...
      if ( skill == null || skill == 'All' ) { // find all users via skills associated with current user and retrieve the count
        ...
        UserSkill.findAllByUser(user).each() { userSkill ->
          ...
          if ( voicemailUser ) {
            ...
            log.warn "InboxMessageService.newAcdMessageCount(): There is no voicemail user configured for skill [${userSkill.skill}]"
          }
        }
      }
        ...
        if ( user ) {
          ...
          log.warn "InboxMessageService.newAcdMessageCount(): There is no voicemail user configured for skill [${skill}]"
        }
      }
