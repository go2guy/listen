...
#external .col-button { width: 60px;}
#internal .col-button,
  ...
  <body>
        <g:if test="${external.size() > 0}">
          <table id="external">
            ...
            <thead>
                <tr><span>
                  <th class="col-pattern" style="width: 235px"><g:message code="page.administration.routing.external.column.pattern"/></th>
        ...
        <g:if test="${internal.size() > 0}">
          <table id="internal">
            <thead>
              <th class="col-pattern" style="width:290px;"><g:message code="page.administration.routing.internal.column.pattern"/></th>
          ...
          <tbody>
            <tr class="add highlighted">
              <g:form controller="administration" action="addDirectInwardDialNumber" method="post">
                ...
                <td class="col-owner" style="width: 535px;"><listen:userSelectForOperator name="owner.id" value="${newDirectInwardDialNumber?.owner?.id}" noSelection="${['':'-- Choose Owner --']}" class="${listen.validationClass(bean: newDirectInwardDialNumber, field: 'owner')}"/></td>
        ...
        <table id="directInwardDial">
          <thead>
            <th class="col-pattern" style="width: 255px;"><g:message code="page.administration.routing.directInwardDial.column.number"/></th>
            ...
            <g:each in="${directInwardDialNumbers}" var="directInwardDialNumber" status="i">
              <tr class="${i % 2 == 0 ? 'even' : 'odd'}">
                <g:form controller="administration" action="updateDirectInwardDialNumber" method="post">
                  ...
                  <td class="col-owner" style="width: 535px;"><listen:userSelectForOperator name="owner.id" value="${directInwardDialNumber.owner.id}" class="${listen.validationClass(bean: directInwardDialNumber, field: 'owner')}"/></td>
                  <td class="col-button">
        ...
        <tbody>
            <tr class="add highlighted">
              <g:form controller="administration" action="addDirectMessageNumber" method="post">
                ...
                <td class="col-owner" style="width: 535px;"><listen:userSelectForOperator name="owner.id" value="${newDirectMessageNumber?.owner?.id}" noSelection="${['':'-- Choose Owner --']}" class="${listen.validationClass(bean: newDirectMessageNumber, field: 'owner')}"/></td>
