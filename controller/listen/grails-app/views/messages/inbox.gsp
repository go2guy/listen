...
div.options .type-tags {
    float: right;
}

div.options .type-tags li {
    background-color: #FFFFFF;
    color: #999999;
    font-size: 12px;
    padding: 2px 5px;
        ...
        <g:each in="${messageList}" var="message">
          <li class="<g:if test="${message.isNew}">new </g:if>message-row <listen:ifIsVoicemail message="${message}">voicemail</listen:ifIsVoicemail><listen:ifIsFax message="${message}">fax</listen:ifIsFax>" id="${message.id}">
            ...
            <listen:ifIsFax message="${message}">
              <div class="playLink" style="display: none;"><listen:megabytes file="${message.file}" unavailable="Size Unknown"/></div>
              <div class="play"></div>
            </listen:ifIsFax>
...
$('.delete-form').submit(function(e) {
    return confirm('Are you sure?');
      ...
      function pollMessages() {
            ...
            $.ajax({
              url: '${request.contextPath}/messages/pollingList?visibleIds=' + visibleIds + '${(request.queryString?.length() > 0) ? "&" + request.queryString : ""}',
              dataType: 'json',
              cache: false,
              success: function(data, textStatus, xhr) {
                   var add = data.add;
                   ...
                   for(var j = 0; j < add.length; j++) {
                        ...
                        if(message.type == 'voicemail') {                        
                            ...
                            clone.find('.playLink').text(message.size);
                            clone.addClass('fax');
